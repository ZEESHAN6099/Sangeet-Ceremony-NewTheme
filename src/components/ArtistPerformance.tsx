...
const ArtistPerformance: React.FC = () => {
  ...
  return (
    <section id="artist" className="section-container panel bg-deep-purple">
      ...
      <div className="relative z-10 flex w-full max-w-3xl md:max-w-6xl flex-col items-center px-4 pb-6 md:pb-12">
        <div className="mb-6 md:mb-12 text-center">
          ...
          <h2 className="font-cinzel text-2xl md:text-4xl text-mustard-yellow text-magical-glow tracking-[0.3em]">
            FAMILY CELEBRATION
          </h2>
        </div>
        <div className="grid items-center gap-8 md:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
          <div className="relative mx-auto w-full max-w-md group">
            ...
            <div ref={cardRef} className="relative rounded-[2rem] border border-mustard-yellow/40 p-3 md:p-6 shadow-[0_24px_60px_rgba(0,0,0,0.45)] bg-gradient-to-b from-deep-purple/80 to-transparent">
              <div className="rounded-[1.7rem] border border-lime-green/30 p-4 md:p-6 overflow-hidden relative artist-card-inner" style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.02), rgba(0,0,0,0.12))' }}>
                <div className="relative aspect-[2/3] md:aspect-[3/4] rounded-[1.4rem] flex items-center justify-center">
                  <div className="text-center px-4 md:px-6">
                    <h3 ref={animatedHeadingRef} className="font-great-vibes text-3xl md:text-6xl text-mustard-yellow drop-shadow-lg mb-4">SOULS & STORIES</h3>
                    <div className="relative h-10 md:h-16">
                      {['LOVE', 'LAUGHTER', 'MUSIC', 'MEMORIES', 'DANCE'].map((w, i) => (
                        <span
                          key={w}
                          ref={(el) => (wordsRef.current[i] = el)}
                          className="absolute inset-0 flex items-center justify-center font-playfair text-lg md:text-3xl text-hot-pink opacity-0 transform scale-90"
                        >
                          {w}
                        </span>
                      ))}
                    </div>
                    <p className="mt-3 text-xs md:text-sm text-lime-green/90">CELEBRATING TOGETHER</p>
                  </div>
                </div>
              </div>
            </div>
